...
def main():
    if len(sys.argv) < 3:
        print(\"usage: telegram_notify.py <filtered_file> <tf>\"); sys.exit(1)
    path, tf = sys.argv[1], sys.argv[2]
    token, chat_id = os.environ.get(\"BOT_TOKEN\",\"\\"), os.environ.get(\"CHAT_ID\",\"\\" )
    if not token or not chat_id:
        print(\"No BOT_TOKEN/CHAT_ID; skipping Telegram notify.\"); sys.exit(0)
    try:
    ...
    if not lines:
        print(\"No OKX lines; skipping Telegram notify.\"); sys.exit(0)
    preview = "\n".join(lines[:8])
    send_message(token, chat_id, f\"🧰 BingX Grid Candidates — {tf}\n{preview}\")
    print(\"Telegram sent.\")
