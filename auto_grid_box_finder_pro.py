...
RECENT_HRS = float(os.getenv("RECENT_HRS","3"))              # aktivasyon: son pencere
...
# Rejim (uzun) eşikleri — sürdürülebilir geniş bant
...
# Aktivasyon (kısa) — hızlı ve düzenli ping-pong
...
def uni_symbols(market, quote="USDT"):
    ...
    for s in r.json()[key]:
        ...
        if market=="futures" and s.get("contractType") not in ("PERPETUAL","CURRENT_QUARTER","NEXT_QUARTER"): continue
        if any(x in s["symbol"] for x in ("UP","DOWN","BULL","BEAR")): continue
        syms.append(s["symbol"])
    return syms

def top_by_quote(symbols, market, topn, volmin):
    url = "https://api.binance.com/api/v3/ticker/24hr" if market=="spot" else "https://fapi.binance.com/fapi/v1/ticker/24hr"
    r=requests.get(url, timeout=30); r.raise_for_status()
    st={d["symbol"]:d for d in r.json() if d.get("symbol") in set(symbols)}
    rows=[]
    for sym,d in st.items():
        try: q=float(d.get("quoteVolume",0)); p=float(d.get("lastPrice",0))
        except: continue
...
def klines(symbol, interval, market, hours):
    ...
    params={"symbol":symbol,"interval":interval,"limit":bars_for_hours(interval,hours)}
    for a in range(5):
        try:
...
def run():
    ...
    if not pool: send("⚠️ Pro Grid Finder: evrende sembol yok."); return
    ...
    for s in pool:
        try:
            ...
            pass
    if not rows: send("⚠️ Pro Grid Finder: veri toplanamadı."); return

    # filtre: rejim + aktivasyon
