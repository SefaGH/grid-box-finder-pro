...
import os, time, math, csv, io, requests
...
def _env_bool(n: str, dflt: bool = False) -> bool:
    v = str(os.environ.get(n, "")).strip().lower()
...
# Telegram knobs
TELEGRAM_ALWAYS_NEAR = _env_bool("TELEGRAM_ALWAYS_NEAR", False)
TELEGRAM_HEALTH_PING = _env_bool("TELEGRAM_HEALTH_PING", True)
TELEGRAM_DEBUG       = _env_bool("TELEGRAM_DEBUG", True)
TELEGRAM_PIN_FAST    = _env_bool("TELEGRAM_PIN_FAST", True)

# CSV knobs
SEND_GRID_CSV        = _env_bool("SEND_GRID_CSV", True)
GRID_CSV_TOP_N       = _env_int("GRID_CSV_TOP_N", 5)  # how many from Top list to include
GRID_CSV_PREFER_S    = _env_bool("GRID_CSV_PREFER_S", True)  # prefer pp[0] symbol at top
...
def crosses_per_hour(closes: List[float]) -> Tuple[float, float]:
    ...
    intervals.sort()
    med = float(intervals[len(intervals) // 2])  # minutes (since 1m bars)
...
def tg_send_message(text: str, *, parse_mode: str=None, disable_preview: bool=True) -> Tuple[bool, int]:
    ...
    for idx, part in enumerate(parts, 1):
        try:
            ...
            ok_all = False
            continue
        ok, mid, desc, code = False, -1, "", None
        try:
            j = r.json()
            ok = bool(j.get("ok", False))
            if ok:
                mid = j.get("result", {}).get("message_id", -1)
        except Exception:
            pass
        if r.status_code != 200 or not ok:
            ...
            try:
                ...
                pass
            print(f"[warn] tg-send error (chunk {idx}/{len(parts)}): status={r.status_code}, ok={ok}, code={code}, desc={desc}, body={body}")
            ok_all = False
        else:
            if TELEGRAM_DEBUG:
                print(f"[tg-send ok] chunk {idx}/{len(parts)} delivered: ok={ok}, mid={mid}")
            last_mid = mid
        time.sleep(0.2)
    return ok_all, last_mid

def tg_pin_message(message_id: int) -> bool:
    token, chat_id = _tg_env()
    if not token or not chat_id or message_id < 0:
        return False
    ...
    try:
        ...
        ok = bool(j.get("ok", False))
        if TELEGRAM_DEBUG:
            print(f"[tg-pin {'ok' if ok else 'err'}] mid={message_id}")
        return ok
    except Exception as e:
        print(f"[warn] tg-pin exception: {e}")
        return False

def tg_send_csv(filename: str, csv_bytes: bytes, caption: str = "") -> bool:
    token, chat_id = _tg_env()
    if not token or not chat_id:
    ...
    try:
        ...
        try:
            ...
            pass
        if TELEGRAM_DEBUG:
            print(f"[tg-file {'ok' if ok else 'err'}] name={filename}, size={len(csv_bytes)}")
        return ok
    except Exception as e:
        print(f"[warn] tg-file exception: {e}")
        return False
...
def make_grid_csv(rows: List[Dict[str, Any]]) -> bytes:
    buf = io.StringIO()
    w = csv.writer(buf)
    w.writerow(["symbol","last","grid_low","grid_high","levels","step","atr_abs","atr_pct","range_pct","adx","mid_cross","drift_pct","tags"])
    for d in rows:
        gl, gh, lv = d.get("grid_lower"), d.get("grid_upper"), int(d.get("levels", 12) or 12)
        step = (gh - gl) / (lv - 1) if (gh and gl and lv and lv>1) else 0.0
        tags = []
        if d.get("pingpong_ok"): tags.append("PING-PONG OK")
        if d.get("fast_ok"): tags.append("FAST S OK")
        tags += d.get("why_tags", [])
        w.writerow([
            d.get("symbol"),
            f"{d.get('last', 0.0):.8g}",
            f"{gl:.8g}" if gl is not None else "",
            f"{gh:.8g}" if gh is not None else "",
            lv,
            f"{step:.8g}",
            f"{d.get('atr_abs', 0.0):.8g}",
            f"{d.get('atr_pct', 0.0):.6f}",
            f"{d.get('range_pct', 0.0):.6f}",
            f"{float(d.get('adx', 0.0)):.2f}",
            int(d.get('midcross', 0) or 0),
            f"{float(d.get('drift_ratio', 0.0)):.6f}",
            " ".join(tags)
        ])
...
def main():
    ...
    if TELEGRAM_HEALTH_PING:
        tg_send_message("🟢 Scanner up — starting scan")
    ...
    if fast_pp:
        ...
        if fst:
            ok, mid = tg_send_message("FAST S OK (wide & quick S)\n" + "\n".join([to_human(d) for d in fst]))
            if TELEGRAM_PIN_FAST and ok:
                if tg_pin_message(mid):
                    print("[tg-pin ok] FAST message pinned")
                else:
                    print("[tg-pin err] FAST pin failed")
            pinned_mid = mid
    else:
        if TELEGRAM_ALWAYS_NEAR:
            tg_send_message("FAST S OK (wide & quick S)\n(şu an eşleşme yok — filtreler sıkı)")
    ...
    if pp:
        ...
        ok, mid = tg_send_message("PING-PONG OK (S davranışı teyitli)\n" + "\n".join([to_human(d) for d in pps]))
        if TELEGRAM_PIN_FAST and not pinned_mid and ok:
            if tg_pin_message(mid):
                print("[tg-pin ok] PING-PONG message pinned")
            else:
                print("[tg-pin err] PING-PONG pin failed")
        if pinned_mid is None:
            pinned_mid = mid
    else:
        if TELEGRAM_ALWAYS_NEAR:
            tg_send_message("PING-PONG OK (S davranışı teyitli)\n(şu an eşleşme yok — filtreler sıkı)")
    ...
    # 4) HTML formatted summary (rich)
    try:
        s_behavior_fmt = _to_fmt_entry(pp[0]) if (pp and len(pp) > 0) else None
        top_fmt  = [_to_fmt_entry(d) for d in (allres[:TOP_SEND] if allres else [])]
        fast_fmt = [_to_fmt_entry(d) for d in (fast_pp[:TOP_FAST] if fast_pp else [])]
        if (s_behavior_fmt or top_fmt or fast_fmt):
            ...
            last_ok, last_mid = False, -1
            for ch in chunks:
                last_ok, last_mid = tg_send_message(ch, parse_mode="HTML", disable_preview=True)
            # (optional) pin HTML block if nothing pinned yet
            if TELEGRAM_PIN_FAST and (pinned_mid is None) and last_ok:
                tg_pin_message(last_mid)
    except Exception as e:
        print(f"[warn] HTML summary error: {e}")

    # 5) CSV attach (optional)
    try:
        if SEND_GRID_CSV:
            csv_rows = []
            pick_syms = set()
            # Prefer s-behavior first
            if GRID_CSV_PREFER_S and pp:
                csv_rows.append(pp[0])
                pick_syms.add(pp[0]["symbol"])
            # Then top list (unique symbols)
            for d in allres[:GRID_CSV_TOP_N]:
                if d["symbol"] not in pick_syms:
                    csv_rows.append(d); pick_syms.add(d["symbol"])
            if csv_rows:
                data = make_grid_csv(csv_rows)
                ok = tg_send_csv("grid_top_candidates.csv", data, caption="Grid CSV (auto) — top candidates")
                if not ok:
                    print("[tg-file err] CSV gönderilemedi")
    except Exception as e:
        print(f"[warn] CSV error: {e}")
